...
export default function AgentStatusPanel() {
  ...
  const totalAgents = data
    ? Object.values(data.domains).flat().length
    : 0;
  const running = data
    ? Object.values(data.domains).flat().filter((a) => a.status === "running").length
    : 0;
  ...
  return (
    <section className="flex flex-col gap-3">
        ...
        <div className="flex flex-col gap-4">
          {DOMAIN_ORDER.map((domain) => {
            ...
            return (
              <div key={domain}>
                <div
                  className={`text-[10px] uppercase tracking-widest mb-2 ${DOMAIN_ACCENT[domain]}`}
                >
      ...
      {!data && !error && (
        <div className="text-xs text-zinc-700 animate-pulse">Loading agent status…</div>
      )}
      {error && (
        <div className="text-xs text-red-500">Cannot reach orchestrator at localhost:7700</div>
      )}
    </section>
  );
}
